...
// Sadece belirli label'lara gore filtre uygula
const SADECE_RAKAM = ["Telefon", "Tel", "TC Kimlik No", "Vergi No"]
const SADECE_HARF = ["Ad", "Soyad"]
...
function Inp({ label, placeholder, type="text", value, onChange, optional }) {
  ...
  const handleChange = (e) => {
    ...
    if (SADECE_RAKAM.includes(label)) {
      val = val.replace(/\D/g, '')
    } else if (SADECE_HARF.includes(label)) {
  ...
  return (
    <div>
      <label style={{ display:"block", fontSize:"11px", fontWeight:"700", color:C.muted, letterSpacing:"0.5px", textTransform:"uppercase", marginBottom:"5px", marginTop:"14px" }}>
        {label}{optional && <span style={{ color:C.dim, fontWeight:"400", textTransform:"none" }}> (istege bagli)</span>}
      </label>
      <input
    ...
    <div style={{ borderRadius:"10px", overflow:"hidden", border:"1px solid "+C.inBorder, marginTop:"8px" }}>
      ...
      {konum && (
        <div style={{ background:C.inBg, padding:"8px 12px", fontSize:"11px", color:C.success, borderTop:"1px solid "+C.inBorder }}>
          Konum secildi: {konum.lat.toFixed(5)}, {konum.lng.toFixed(5)}
        </div>
      )}
    </div>
  )
...
function YasalModal({ tip, onClose }) {
  ...
  const basliklar = {
    kullanim:{icon:"📋",baslik:"Kullanim Kosullari"},
    kvkk:{icon:"🔐",baslik:"KVKK Aydinlatma Metni"},
    gizlilik:{icon:"🔏",baslik:"Gizlilik Politikasi"},
    iade:{icon:"↩️",baslik:"Iade & Iptal"},
  }
  const info = basliklar[tip] || { icon:"📄", baslik:"Yasal Metin" }
  useEffect(() => {
    sb.from("ayarlar").select("deger").eq("id","yasal_"+tip).maybeSingle()
      .then(({ data }) => { setMetin(data?.deger || "Bu metin henuz eklenmemistir."); setLoading(false) })
  }, [tip])
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.88)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:"16px" }} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.card, border:"1px solid "+C.border, borderRadius:"18px", width:"100%", maxWidth:"640px", maxHeight:"88vh", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ overflowY:"auto", padding:"24px", flex:1 }}>
          {loading
            ? <div style={{ textAlign:"center", color:C.dim, padding:"40px" }}>Yukleniyor...</div>
...
function FooterModal({ tip, onClose }) {
  ...
  const basliklar = {
    ...
    iletisim:{icon:"📨",baslik:"Iletisim"},
    sikayet:{icon:"⚠️",baslik:"Sikayet"},
    oneri:{icon:"💡",baslik:"Oneri & Istek"},
    sss:{icon:"❓",baslik:"Sik Sorulan Sorular"}
  }
  const sss = [
    { s:"Nasil kurye olabilirim?", c:"Anasayfadan 'Kurye Ol' butonuna tiklayarak ucretsiz kayit olabilirsiniz." },
    { s:"Odeme ne zaman yapilir?", c:"Tamamlanan teslimatların odemeleri ayin ilk is gunu hesabiniza aktarilir." },
    { s:"Hangi araclarla calisabilirim?", c:"Motorsiklet, otomobil, kamyonet ve kamyon ile calisabilirsiniz." },
    { s:"Referans sistemi nasil calisir?", c:"Davet kodunuzla kayit olan kisilerin islemlerinden prim kazanirsiniz." },
    { s:"Teslimat bolgesi var mi?", c:"Su an tum Turkiye genelinde hizmet vermekteyiz." }
  ]
  const gonder = async () => {
    ...
    setMsg({ type:"success", text:"Talebiniz alindi!" })
    setYukleniyor(false)
      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:C.card, border:"1px solid "+C.border, borderRadius:"18px", padding:"28px", width:"100%", maxWidth:"480px", maxHeight:"85vh", overflowY:"auto" }}>
        ...
        {tip==="kimiz" && (
          <div style={{ fontSize:"13px", color:C.muted, lineHeight:"2" }}>
            <p><b style={{color:C.text}}>MotoTeslim</b>, Turkiye genelinde hizli ve guvenilir teslimat hizmeti sunan yerli bir platformdur.</p>
        ...
        {["iletisim","sikayet","oneri"].includes(tip) && (
          <div>
            {[
              { l:"Adiniz Soyadiniz *", k:"ad", ph:"Adiniz" },
              { l:"Telefon", k:"tel", ph:"05xx xxx xx xx" },
              { l:"Mesajiniz *", k:"mesaj", ph:"Mesajinizi yazin...", multi:true }
            ].map(f => (
            ...
            <button onClick={gonder} disabled={yukleniyor} style={{ ...btnP, width:"100%", boxShadow:"none", opacity:yukleniyor?0.7:1 }}>
...
function Sozlesmeler({ sozlesme, setSozlesme, kvkk, setKvkk, onYasal }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:"8px", marginTop:"12px" }}>
      {[
        { s:sozlesme, fn:setSozlesme, tip:"kullanim", label:"Kullanim Kosullari ve Satis Sozlesmesini" },
        { s:kvkk, fn:setKvkk, tip:"kvkk", label:"KVKK Aydinlatma Metnini" }
      ].map(x => (
...
function KuryeGiris({ onClose, onSuccess }) {
  ...
  const giris = async () => {
    if (!form.tel||!form.sifre) { setAlert({ type:"error", msg:"Telefon ve sifre zorunludur." }); return }
    ...
    if (error||!data) {
      setAlert({ type:"error", msg:"Telefon veya sifre hatali." })
    } else {
  ...
  return (
    <div style={modalStyle} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={mBox}>
        ...
        <div style={{ fontSize:"18px", fontWeight:"800", marginBottom:"4px" }}>🏍 Kurye Girisi</div>
        <div style={{ fontSize:"12px", color:C.muted, marginBottom:"16px" }}>Hesabina giris yap</div>
        <Inp label="Telefon" placeholder="0532 123 4567" value={form.tel} onChange={set("tel")}/>
        <Inp label="Sifre" placeholder="••••••••" type="password" value={form.sifre} onChange={set("sifre")}/>
...
function SifreUnuttum({ tip, onClose }) {
  ...
  const gonder = async () => {
    if (!tel.trim()) { setAlert({ type:"error", msg:"Telefon numaranizi girin." }); return }
    setLoading(true); setAlert({ type:"", msg:"" })
    ...
    if (!data) {
      setAlert({ type:"error", msg:"Bu telefon numarasi kayitli degil." })
    } else {
      const mesaj = `MotoTeslim: Sayin ${data.ad}, sifreniz: ${data.sifre}`
      const ok = await smsSend(t, mesaj)
      if (ok) {
        setAlert({ type:"success", msg:"Sifreniz SMS ile gonderildi!" })
        setTimeout(onClose, 2500)
      } else {
        setAlert({ type:"error", msg:"SMS gonderilemedi. Lutfen tekrar deneyin." })
      }
    }
    setLoading(false)
  }
  ...
  return (
    <div style={modalStyle} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{ ...mBox, maxWidth:"420px" }}>
        <button style={{ position:"absolute", top:"14px", right:"16px", background:"none", border:"none", color:C.muted, fontSize:"22px", cursor:"pointer" }} onClick={onClose}>✕</button>
        <div style={{ fontSize:"18px", fontWeight:"800", marginBottom:"4px" }}>🔑 Sifremi Unuttum</div>
        <div style={{ fontSize:"12px", color:C.muted, marginBottom:"16px" }}>Kayitli telefon numaraniza sifreniz SMS ile gonderilecektir.</div>
        ...
        <button onClick={gonder} disabled={loading} style={{ ...btnP, width:"100%", marginTop:"16px", boxShadow:"none", opacity:loading?0.7:1 }}>
          {loading ? "Gonderiliyor..." : "📱 SMS Gonder"}
        </button>
      </div>
    </div>
  )
...
function KuryeKayit({ onClose, onSuccess }) {
  ...
  const otpGonder = async () => {
    ...
    if (mevcut) { setAlert({ type:"error", msg:"Bu telefon zaten kayitli." }); setLoading(false); return }
    const kod = otpUret()
    setOtpKod(kod)
    ...
    if (ok) {
      setAlert({ type:"success", msg:"Dogrulama kodu gonderildi!" })
      setGeriSayim(60)
      setStep(4)
    } else {
      setAlert({ type:"error", msg:"SMS gonderilemedi. Lutfen tekrar deneyin." })
  ...
  const otpDogrula = async () => {
    if (otpGirilen !== otpKod) { setAlert({ type:"error", msg:"Kod hatali! Tekrar deneyin." }); return }
    setLoading(true)
    const tel = telFormat(form.tel)
    ...
    if (error) {
      setAlert({ type:"error", msg:"Kayit hatasi: "+error.message })
    } else {
  ...
  const adimlar = ["Kisisel","Arac","Vergi","Dogrula"]
  ...
  return (
    <div style={modalStyle} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={mBox}>
        ...
        {step===1 && (
          <>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
              ...
              <div><Inp label="Soyad" placeholder="Yilmaz" value={form.soyad} onChange={set("soyad")}/>{err.soyad&&<div style={{color:C.error,fontSize:"10px"}}>{err.soyad}</div>}</div>
            </div>
            ...
            <div><Inp label="Sifre" placeholder="En az 6 karakter" type="password" value={form.sifre} onChange={set("sifre")}/>{err.sifre&&<div style={{color:C.error,fontSize:"10px"}}>{err.sifre}</div>}</div>
        ...
        {step===2 && (
          <>
            <div style={{ fontSize:"18px", fontWeight:"800", marginBottom:"12px" }}>🚗 Arac Bilgileri</div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
              <Inp label="Arac Markasi" placeholder="Honda" value={form.marka} onChange={set("marka")} optional/>
              <Inp label="Arac Modeli" placeholder="PCX 125" value={form.model} onChange={set("model")} optional/>
            </div>
        ...
        {step===3 && (
          <>
            ...
            <Inp label="Vergi Dairesi" placeholder="Kadikoy Vergi Dairesi" value={form.vergiD} onChange={set("vergiD")} optional/>
        ...
        {step===4 && (
          <>
            <div style={{ fontSize:"18px", fontWeight:"800", marginBottom:"8px" }}>📱 Telefon Dogrulama</div>
            <div style={{ background:"rgba(34,197,94,0.08)", border:"1px solid rgba(34,197,94,0.2)", borderRadius:"10px", padding:"12px", marginBottom:"12px", fontSize:"12px", color:C.success }}>
              <b>{form.tel}</b> numarasina 6 haneli kod gonderildi.
            </div>
            <label style={{ display:"block", fontSize:"11px", fontWeight:"700", color:C.muted, letterSpacing:"0.5px", textTransform:"uppercase", marginBottom:"5px", marginTop:"14px" }}>Dogrulama Kodu</label>
            ...
            <div style={{ textAlign:"center", marginTop:"10px" }}>
              {geriSayim > 0
                ? <span style={{ fontSize:"12px", color:C.dim }}>Yeniden gonder: {geriSayim}s</span>
                : <span onClick={otpGonder} style={{ fontSize:"12px", color:C.accent, cursor:"pointer", textDecoration:"underline" }}>Kodu tekrar gonder</span>
              }
            </div>
          </>
        )}
        ...
        <div style={{ display:"flex", gap:"10px", marginTop:"20px" }}>
          ...
          {step===1 && <button onClick={next} style={{ ...btnP, flex:1, padding:"11px", boxShadow:"none" }}>Devam →</button>}
          {step===2 && <button onClick={next} style={{ ...btnP, flex:1, padding:"11px", boxShadow:"none" }}>Devam →</button>}
          {step===3 && (
            <button onClick={otpGonder} disabled={loading||!sozlesme||!kvkk}
              ...
              {loading ? "Gonderiliyor..." : "📱 SMS Kodu Gonder"}
            </button>
          )}
          {step===4 && (
            <button onClick={otpDogrula} disabled={loading||otpGirilen.length<6}
              ...
              {loading ? "Dogrulanıyor..." : "✅ Kaydi Tamamla"}
            </button>
          )}
        </div>
      </div>
...
function GondericiModal({ onClose, onSuccess }) {
  ...
  const otpGonder = async () => {
    ...
    if (mevcut) { setAlert({ type:"error", msg:"Bu telefon zaten kayitli. Giris yapin." }); setLoading(false); return }
    const kod = otpUret()
    setOtpKod(kod)
    ...
    if (ok) {
      ...
      setAlert({ type:"error", msg:"SMS gonderilemedi." })
    }
    setLoading(false)
  }

  const otpDogrula = async () => {
    if (otpGirilen !== otpKod) { setAlert({ type:"error", msg:"Kod hatali!" }); return }
    setLoading(true)
    const tel = telFormat(form.tel)
  ...
  return (<>
    <div style={modalStyle} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={mBox}>
        <button style={{ position:"absolute", top:"14px", right:"16px", background:"none", border:"none", color:C.muted, fontSize:"22px", cursor:"pointer" }} onClick={onClose}>✕</button>
        <div style={{ fontSize:"18px", fontWeight:"800", marginBottom:"4px" }}>📦 Paket Gonder</div>
        <div style={{ fontSize:"12px", color:C.muted, marginBottom:"16px" }}>Giris yap veya misafir olarak devam et</div>

        <div style={{ display:"flex", gap:"6px", marginBottom:"20px", background:C.inBg, borderRadius:"10px", padding:"4px" }}>
          {[["giris","Giris Yap"],["kayit","Kayit Ol"],["misafir","Misafir"]].map(([v,l]) => (
        ...
        {tab==="giris" && (
          <>
            ...
            <div style={{ textAlign:"center", marginTop:"12px" }}>
              <span onClick={()=>setSifreModal(true)} style={{ fontSize:"12px", color:C.muted, cursor:"pointer", textDecoration:"underline" }}>Sifremi unuttum</span>
            </div>
          </>
        )}
        ...
        {tab==="kayit" && otpAdim && (
          <>
            ...
            <button onClick={otpDogrula} disabled={loading||otpGirilen.length<6}
              ...
              {loading ? "Dogrulanıyor..." : "✅ Kayit Tamamla"}
            </button>
          </>
        )}
        ...
        {tab==="misafir" && (
          <>
            <div style={{ background:C.accentGlow, border:"1px solid rgba(232,80,10,0.3)", borderRadius:"10px", padding:"10px 13px", fontSize:"12px", color:C.accentL, marginBottom:"14px" }}>
              👋 Hesap acmadan da gonderim yapabilirsin.
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
              <div><Inp label="Ad" placeholder="Ahmet" value={form.ad} onChange={set("ad")}/>{err.ad&&<div style={{color:C.error,fontSize:"10px"}}>{err.ad}</div>}</div>
              <div><Inp label="Soyad" placeholder="Yilmaz" value={form.soyad} onChange={set("soyad")}/>{err.soyad&&<div style={{color:C.error,fontSize:"10px"}}>{err.soyad}</div>}</div>
...
function PaketGonder({ user, onClose }) {
  ...
  const v1 = () => {
    ...
    if (farkliAdres||!user||!user.adres) {
      ...
      if (!gonKonum) e.gonKonum="Haritadan konum secin"
    }
    if (!form.alAd) e.alAd="Zorunlu"
    if (!form.alTel) e.alTel="Zorunlu"
    if (!alKonum) e.alKonum="Haritadan konum secin"
    if (!form.agirlik) e.agirlik="Agirlik zorunludur"
    return e
  }
  ...
  if (done) return (
    <div style={{ maxWidth:"500px", margin:"40px auto", padding:"0 20px", textAlign:"center" }}>
      ...
      <div style={{ fontSize:"20px", fontWeight:"900", marginBottom:"8px" }}>Siparis Olusturuldu!</div>
      <div style={{ background:C.card, borderRadius:"14px", padding:"20px", border:"1px solid "+C.border, textAlign:"left", marginBottom:"20px" }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", fontSize:"12px" }}>
          <div style={{ color:C.muted }}>Siparis No:</div><div style={{ fontWeight:"700" }}>#MT-{Math.floor(Math.random()*9000+1000)}</div>
          ...
          <div style={{ color:C.muted }}>Arac:</div><div style={{ fontWeight:"700" }}>{aracLabel}</div>
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth:"600px", margin:"0 auto", padding:"20px" }}>
      <div style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"20px" }}>
        ...
        <div style={{ fontSize:"18px", fontWeight:"800" }}>📦 Paket Gonder</div>
      </div>
      <div style={{ display:"flex", gap:"4px", marginBottom:"24px" }}>
        {["Adres & Agirlik","Odeme"].map((s,i) => (
          <div key={i} style={{ flex:1, textAlign:"center" }}>
      ...
      {step===1 && (
        <div>
          <div style={{ background:C.card2, borderRadius:"12px", padding:"16px", marginBottom:"12px", border:"1px solid "+C.border }}>
            <div style={{ fontSize:"12px", fontWeight:"700", color:C.accent, marginBottom:"10px" }}>📍 GONDERICI</div>
            {user&&user.adres ? (
              <div>
                <div style={{ background:"rgba(34,197,94,0.08)", border:"1px solid rgba(34,197,94,0.2)", borderRadius:"10px", padding:"12px", marginBottom:"12px" }}>
                  <div style={{ fontSize:"11px", color:C.success, fontWeight:"700", marginBottom:"4px" }}>✓ Kayitli Hesabiniz</div>
                ...
                <label style={{ display:"flex", alignItems:"center", gap:"10px", cursor:"pointer", padding:"10px 14px", borderRadius:"9px", border:"1px dashed "+(farkliAdres?C.accent:C.border), background:farkliAdres?"rgba(232,80,10,0.06)":"transparent" }}>
                  ...
                  <span style={{ fontSize:"13px", color:farkliAdres?C.accent:C.muted, fontWeight:farkliAdres?"700":"500" }}>📍 Farkli adresten gonderim</span>
                </label>
              </div>
            ) : null}
            {(farkliAdres||!user||!user.adres) && (
              <div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
                  ...
                  <Inp label="Soyad" placeholder="Yilmaz" value={form.gonSoyad} onChange={set("gonSoyad")}/>
                </div>
            ...
            <div style={{ marginTop:"12px" }}>
              <button onClick={()=>setHaritaAcik(haritaAcik==="gon"?null:"gon")}
                ...
                {gonKonum?"✅ Konum Secildi — Degistir":"🗺️ Haritadan Konum Sec"}
              </button>
          ...
          <div style={{ background:C.card2, borderRadius:"12px", padding:"16px", marginBottom:"12px", border:"1px solid "+C.border }}>
            ...
            <div style={{ marginTop:"12px" }}>
              <button onClick={()=>setHaritaAcik(haritaAcik==="al"?null:"al")}
                ...
                {alKonum?"✅ Konum Secildi — Degistir":"🗺️ Haritadan Konum Sec"}
              </button>
          ...
          <div style={{ background:C.card2, borderRadius:"12px", padding:"16px", marginBottom:"12px", border:"1px solid "+C.border }}>
            <div style={{ fontSize:"12px", fontWeight:"700", color:C.muted, marginBottom:"10px" }}>⚖️ PAKET BILGISI</div>
            <div><Inp label="Agirlik (kg)" placeholder="5" type="number" value={form.agirlik} onChange={set("agirlik")}/>{err.agirlik&&<div style={{color:C.error,fontSize:"10px"}}>{err.agirlik}</div>}</div>
            {kg>20 && (
              <div>
                <div style={{ background:"rgba(245,158,11,0.1)", border:"1px solid rgba(245,158,11,0.3)", borderRadius:"10px", padding:"10px", fontSize:"12px", color:C.warn, marginTop:"10px" }}>⚠️ 20 kg uzeri icin olcu giriniz.</div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"10px" }}>
                  ...
                  <Inp label="Yukseklik" placeholder="20" type="number" value={form.yukseklik} onChange={set("yukseklik")}/>
                </div>
              </div>
            )}
            <Inp label="Aciklama" placeholder="Kirılgan..." value={form.aciklama} onChange={set("aciklama")} optional/>
          </div>
          ...
          {gonKonum&&alKonum&&mesafe>0&&fiyat>0 && (
            <div style={{ background:"linear-gradient(135deg,rgba(232,80,10,0.12),rgba(30,58,96,0.25))", border:"1px solid "+C.accent, borderRadius:"14px", padding:"18px", marginBottom:"12px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"10px" }}>
                <div style={{ fontSize:"12px", fontWeight:"700", color:C.accent }}>💰 FIYAT HESAPLAMA</div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px", fontSize:"12px", marginBottom:"10px" }}>
                ...
                <div style={{ color:C.muted }}>Kurye kazanci:</div><div style={{ fontWeight:"600", color:C.success }}>₺{fiyat-komisyon}</div>
              </div>
              <div style={{ borderTop:"1px solid rgba(255,255,255,0.1)", paddingTop:"10px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <span style={{ fontSize:"13px", fontWeight:"700" }}>Toplam Ucret</span>
      ...
      {step===2 && (
        <div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"10px", marginBottom:"16px" }}>
            {[{v:"kart",icon:"💳",title:"Yeni Kart",sub:"Visa, Mastercard, Troy"},{v:"kayitli",icon:"⚡",title:"Kayitli Kart",sub:"•••• 4521"},{v:"cuzdan",icon:"👛",title:"MotoTeslim Cuzdan",sub:"Bakiye: ₺850"}].map(o=>(
          ...
          {form.odemeYontemi==="kart" && (
            <div style={{ background:C.card2, borderRadius:"12px", padding:"16px", border:"1px solid "+C.border }}>
              <Inp label="Kart Numarasi" placeholder="1234 5678 9012 3456" value={form.kartNo} onChange={set("kartNo")}/>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"8px", marginTop:"14px", padding:"10px 12px", background:"rgba(30,58,96,0.3)", borderRadius:"9px", border:"1px solid "+C.navy }}>
            <span>🔒</span><span style={{ fontSize:"11px", color:C.dim }}>Tum odemeler <b style={{color:C.muted}}>PayTR</b> ile 256-bit SSL sifreli.</span>
          </div>
        </div>
      )}
      ...
      <div style={{ display:"flex", gap:"10px", marginTop:"22px" }}>
        ...
        <button onClick={next} style={{ ...btnP, flex:1, boxShadow:"none", padding:"12px" }}>
          {step===2 ? "💳 Odemeyi Tamamla" : "Odemeye Gec →"}
        </button>
      </div>
    </div>
  )
...
function GecmisTeslimatlar({ userId }) {
  ...
  if (loading) return <div style={{textAlign:"center",color:C.muted,padding:"30px"}}>Yukleniyor...</div>
  if (!liste.length) return <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:"13px",padding:"40px",textAlign:"center",color:C.muted,fontSize:"13px"}}>Henuz tamamlanan teslimat yok.</div>
  return (
    <div>
      {liste.map(s => (
        <div key={s.id} style={{background:C.card,border:"1px solid "+C.border,borderRadius:"12px",padding:"14px",marginBottom:"10px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:"12px",fontWeight:"600"}}>{s.gon_adres||"Konum kayitli"}</div>
            <div style={{fontSize:"11px",color:C.muted,marginTop:"2px"}}>→ {s.al_adres||"Konum kayitli"}</div>
...
function KuryeDash({ user, onLogout }) {
  ...
  const kabul = async (s) => {
    ...
    window.open(lat&&lng ? `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&travelmode=driving` : `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent((s.gon_adres||"")+", Turkiye")}&travelmode=driving`, "_blank")
  }
  ...
  const paketiAldim = async () => {
    ...
    window.open(lat&&lng ? `https://www.google.com/maps/dir/?api=1&destination=${lat},${lng}&travelmode=driving` : `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent((aktifSiparis.al_adres||"")+", Turkiye")}&travelmode=driving`, "_blank")
    const ok = await smsSend(aktifSiparis.al_tel, `MotoTeslim teslimat kodunuz: ${kod} - Kuryeye bu kodu veriniz.`)
    alert(ok ? "Teslim kodu SMS ile gonderildi!" : "Teslimat basladi! Kod: "+kod)
  }

  const teslimEt = async () => {
    if (teslimKodu!==aktifSiparis.teslim_kodu) { setTeslimHata("Kod hatali!"); return }
    ...
    alert("Tamamlandi! ₺"+kazanc+" eklendi!")
  }
  ...
  return (
    <div style={{ maxWidth:"880px", margin:"0 auto", padding:"24px 20px" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"24px" }}>
        ...
        <button onClick={onLogout} style={{ ...btnS, fontSize:"12px", padding:"7px 14px" }}>Cikis</button>
      </div>

      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:"12px", marginBottom:"20px" }}>
        {[
          { l:"Bu Ay Teslimat", v:stats.buAy, s:"adet" },
          { l:"Toplam Teslimat", v:stats.toplam, s:"tum zamanlar" },
          { l:"Toplam Kazanc", v:"₺"+stats.kazanc, s:"net" },
          { l:"Sonraki Odul", v:sonrakiOdul+" teslimat", s:"Her 50'de ₺50" }
        ].map(x => (
      ...
      {aktifSiparis && (
        <div style={{ background:"rgba(34,197,94,0.08)", border:"2px solid "+C.success, borderRadius:"14px", padding:"20px", marginBottom:"20px" }}>
          <div style={{ fontSize:"14px", fontWeight:"800", color:C.success, marginBottom:"14px" }}>
            {aktifSiparis.durum==="kabul_edildi" ? "🏍 Gonderici Adresine Git" : "📦 Alıcıya Teslim Et"}
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"16px", fontSize:"12px" }}>
            <div style={{ background:"rgba(232,80,10,0.1)", borderRadius:"10px", padding:"12px" }}>
              <div style={{ color:C.accent, fontWeight:"700", marginBottom:"4px" }}>📍 GONDERICI</div>
          ...
          {aktifSiparis.durum==="kabul_edildi" && (
            <div style={{ display:"flex", gap:"10px" }}>
              <button onClick={()=>{const l=aktifSiparis.gon_lat,g=aktifSiparis.gon_lng;window.open(l&&g?`https://www.google.com/maps/dir/?api=1&destination=${l},${g}&travelmode=driving`:`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent((aktifSiparis.gon_adres||"")+", Turkiye")}&travelmode=driving`,"_blank")}}
                style={{ ...btnS, flex:1, fontSize:"13px", padding:"10px" }}>🗺️ Haritayi Ac</button>
              <button onClick={paketiAldim} style={{ ...btnP, flex:1, fontSize:"13px", padding:"10px", boxShadow:"none" }}>📦 Paketi Aldim</button>
            </div>
          )}
          {aktifSiparis.durum==="yolda" && (
            <div>
              <div style={{ background:"rgba(34,197,94,0.05)", border:"1px solid rgba(34,197,94,0.2)", borderRadius:"10px", padding:"12px", marginBottom:"12px", fontSize:"12px", color:C.success }}>
                📱 Teslim kodu musteriye SMS ile gonderildi.
              </div>
              <div style={{ display:"flex", gap:"10px", alignItems:"center", marginBottom:"10px" }}>
                ...
                <button onClick={()=>{const l=aktifSiparis.al_lat,g=aktifSiparis.al_lng;window.open(l&&g?`https://www.google.com/maps/dir/?api=1&destination=${l},${g}&travelmode=driving`:`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent((aktifSiparis.al_adres||"")+", Turkiye")}&travelmode=driving`,"_blank")}}
      ...
      <div style={{ display:"flex", gap:"6px", marginBottom:"16px", background:C.inBg, borderRadius:"10px", padding:"4px" }}>
        {[["bekleyen","Bekleyen Siparisler"],["gecmis","Gecmis"]].map(([v,l]) => (
      ...
      {tab==="bekleyen" && (
        <div>
          {loading && <div style={{textAlign:"center",color:C.muted,padding:"40px"}}>Yukleniyor...</div>}
          {!loading&&!siparisler.length&&!aktifSiparis && <div style={{background:C.card,border:"1px solid "+C.border,borderRadius:"13px",padding:"50px",textAlign:"center",color:C.muted,fontSize:"13px"}}>🏍 Su an bekleyen siparis yok.</div>}
          {siparisler.map(s => (
            <div key={s.id} style={{background:C.card,border:"1px solid "+C.border,borderRadius:"13px",padding:"16px",marginBottom:"12px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px",fontSize:"12px",marginBottom:"10px"}}>
                <div>
                  <div style={{color:C.accent,fontWeight:"700",fontSize:"10px",marginBottom:"2px"}}>📍 GONDERICI</div>
              ...
              <button onClick={()=>kabul(s)} disabled={!!aktifSiparis}
                ...
                {aktifSiparis ? "Once aktif teslimatı tamamla" : "✅ Kabul Et"}
              </button>
            </div>
          ))}
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    sb.from("ayarlar").select("deger").eq("id","kariyer_metin").maybeSingle()
      .then(({ data }) => setKariyerMetin(data?.deger||"MotoTeslim ile hem calis hem kazan!\n\nSen de kurye olarak sisteme katil, teslimat yap ve referanslarinla ekstra gelir elde et."))
  }, [])
    ...
    <div style={{ minHeight:"100vh", background:C.bg }}>
      <nav style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 32px", borderBottom:"1px solid "+C.border, background:"rgba(10,14,26,0.95)", backdropFilter:"blur(12px)", position:"sticky", top:0, zIndex:100 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
          ...
          <button onClick={logout} style={{ ...btnS, fontSize:"12px", padding:"6px 12px" }}>Cikis</button>
        </div>
      </nav>
      ...
      {paketAc
        ...
        : (
          <div style={{ maxWidth:"500px", margin:"60px auto", padding:"0 20px", textAlign:"center" }}>
            ...
            <div style={{ color:C.muted, marginBottom:"28px" }}>{user.type==="misafir"?"Misafir olarak giris yaptiniz":"Hesabiniza giris yaptiniz"}</div>
            <button onClick={()=>setPaketAc(true)} style={{ ...btnP, width:"100%", marginBottom:"12px" }}>📦 Paket Gonder</button>
            <button onClick={logout} style={{ ...btnS, width:"100%" }}>← Ana Sayfaya Don</button>
          </div>
        )
      }
    </div>
    ...
    <div style={{ minHeight:"100vh", background:C.bg, backgroundImage:"radial-gradient(ellipse at 20% 50%,rgba(232,80,10,0.05) 0%,transparent 60%),radial-gradient(ellipse at 80% 20%,rgba(30,58,96,0.3) 0%,transparent 50%)" }}>
      <nav style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 32px", borderBottom:"1px solid "+C.border, background:"rgba(10,14,26,0.95)", backdropFilter:"blur(12px)", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ display:"flex", alignItems:"center", gap:"16px" }}>
          <div style={{ fontSize:"20px", fontWeight:"900", color:C.accent }}>🏍 MotoTeslim</div>
          <button onClick={()=>setModal("kariyer")} style={{ padding:"6px 14px", borderRadius:"20px", border:"1px solid rgba(232,80,10,0.4)", background:"rgba(232,80,10,0.08)", color:C.accent, fontSize:"12px", fontWeight:"700", cursor:"pointer" }}>🏅 Kariyer Plani</button>
        </div>
        <div>
          <button onClick={()=>setModal("giris")} style={{ padding:"8px 16px", borderRadius:"8px", cursor:"pointer", fontSize:"13px", fontWeight:"700", border:"1px solid "+C.border, background:"transparent", color:C.muted, marginLeft:"8px" }}>Kurye Girisi</button>
      ...
      <div style={{ textAlign:"center", padding:"60px 20px 50px" }}>
        ...
        <div style={{ display:"inline-block", padding:"5px 14px", borderRadius:"20px", border:"1px solid "+C.accent, color:C.accent, fontSize:"11px", fontWeight:"700", letterSpacing:"1px", textTransform:"uppercase", marginBottom:"20px" }}>⚡ mototeslim.com — Hizli Teslimat</div>
        <h1 style={{ fontSize:"clamp(28px,5vw,52px)", fontWeight:"900", lineHeight:"1.1", marginBottom:"16px", letterSpacing:"-1px", color:C.text }}>
          Paketini Gonder,<br/><span style={{ color:C.accent }}>MotoTeslim Halleder</span>
        </h1>
        <p style={{ fontSize:"15px", color:C.muted, maxWidth:"460px", margin:"0 auto 36px", lineHeight:"1.7" }}>
          Yakınındaki kuryelere aninda bildirim. Agirliga gore uygun arac, seffaf fiyat, guvenli odeme.
        </p>
        <div style={{ display:"flex", gap:"12px", justifyContent:"center", flexWrap:"wrap" }}>
          <button onClick={()=>setModal("gonder")} style={btnP}>📦 Paket Gonder</button>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(210px,1fr))", gap:"18px", padding:"0 32px 70px", maxWidth:"1100px", margin:"0 auto" }}>
        {[
          { i:"⚖️", t:"Akilli Fiyatlandirma", d:"Agirlik ve mesafeye gore otomatik hesaplama." },
          { i:"📍", t:"En Yakin Kurye", d:"Once yakindaki kuryeler bildirim alir." },
          { i:"🔐", t:"Guvenli Teslimat", d:"4 haneli teslim kodu ile guvenli el degistirme." },
          { i:"💳", t:"PayTR ile Odeme", d:"Kart ile guvenli odeme. Kurye ucreti otomatik transfer." }
        ].map(f => (
        ...
        <div style={{ maxWidth:"1000px", margin:"0 auto" }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:"32px", marginBottom:"32px" }}>
            <div>
              ...
              <div style={{ fontSize:"12px", color:C.dim, lineHeight:"1.7" }}>Hizli, guvenli ve seffaf teslimat platformu.</div>
            </div>
            {[
              { baslik:"Destek", linkler:[{l:"Iletisim",m:"iletisim"},{l:"Sikayet",m:"sikayet"},{l:"Oneri",m:"oneri"},{l:"SSS",m:"sss"}] },
              { baslik:"Yasal", linkler:[{l:"Kullanim Kosullari",m:"kullanim"},{l:"KVKK",m:"kvkk"},{l:"Gizlilik",m:"gizlilik"},{l:"Iade & Iptal",m:"iade"}] }
            ].map(g => (
              <div key={g.baslik}>
          ...
          <div style={{ borderTop:"1px solid "+C.border, paddingTop:"20px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"10px" }}>
            <div style={{ fontSize:"11px", color:C.dim }}>© 2025 MotoTeslim. Tum haklari saklidir.</div>
            <div style={{ fontSize:"11px", color:C.dim }}>Guvenli odeme: PayTR 256-bit SSL</div>
          </div>
        </div>
      </footer>
      ...
      {modal==="kariyer" && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:"16px" }} onClick={()=>setModal(null)}>
          <div onClick={e=>e.stopPropagation()} style={{ background:C.card, border:"1px solid "+C.border, borderRadius:"18px", padding:"28px", width:"100%", maxWidth:"560px", maxHeight:"85vh", overflowY:"auto" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"20px" }}>
              <div style={{ fontSize:"17px", fontWeight:"800" }}>🏅 Kariyer Plani</div>
